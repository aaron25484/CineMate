...
export const uploadPosterWithCloudinary = async (req: Request, res: Response) => {
  try {
  ...
  if(poster){
    if("tempFilePath" in poster){
      console.log('Uploading poster:', poster.tempFilePath);

      posterUploaded = await uploadImage(poster.tempFilePath)
      await fs.unlink(poster.tempFilePath)
      console.log('Poster uploaded successfully:', posterUploaded);
